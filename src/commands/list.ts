...
import ora from 'ora';
import type { ListCommandOptions } from '../types/config.d.js';
 ...
 */
async function collectServerInfo(options: ListCommandOptions): Promise<ServerInfo[]> {
  ...
  try {
    ...
    for (const agent of agents) {
      ...
      try {
        ...
        const scopes: ConfigScope[] = ['user', 'project'];
...
async function getServerInfo(
  ...
  try {
    ...
    let status: 'active' | 'inactive' | 'disabled' | 'error' = 'active';
    let transport = 'stdio';
 ...
 */
function applyFilters(servers: ServerInfo[], options: ListCommandOptions): ServerInfo[] {
 ...
 */
async function displayServers(servers: ServerInfo[], options: ListCommandOptions): Promise<void> {
 ...
 */
async function displayTable(servers: ServerInfo[], options: ListCommandOptions): Promise<void> {
 ...
 */
function formatServerName(server: ServerInfo): string {
 ...
 */
function formatAgent(agent: AgentType): string {
  const colors = {
    claude: chalk.blue,
    vscode: chalk.cyan,
...
function formatTransport(transport: string): string {
  ...
  const icon = icons[transport as keyof typeof icons] || '❓';
 ...
 */
async function formatHealth(server: ServerInfo): Promise<string> {
 ...
 */
function displaySummary(filteredServers: ServerInfo[], totalServers: number): void {
