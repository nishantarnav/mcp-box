...
import type { SecurityConfig } from '../types/config.d.js';
 ...
 */
export async function keychainSetCommand(
  account: string, 
  ...
  try {
    ...
    if (options.interactive !== false) {
      ...
      if (answers.description || answers.tags) {
        ...
        if (answers.tags) {
          metadata.tags = answers.tags.split(',').map((tag: string) => tag.trim()).filter(Boolean);
        }
 ...
 */
export async function keychainGetCommand(
  account: string, 
  options: KeychainCommandOptions
): Promise<void> {
 ...
 */
export async function keychainDeleteCommand(
  account: string, 
  options: KeychainCommandOptions
): Promise<void> {
 ...
 */
export async function keychainListCommand(options: KeychainCommandOptions): Promise<void> {
 ...
 */
export async function generateTokenCommand(
  length = 32,
