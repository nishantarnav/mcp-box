...
export class SecurityManager {
     ...
     */
    async validateEnvironmentConfig(envConfig) {
        const result = {
            valid: true,
            errors: [],
            warnings: [],
     ...
     */
    async storeSecret(account, password, metadata) {
        if (!this.config.keychainIntegration) {
    ...
    sanitizeConfig(config) {
        ...
        this.walkObject(sanitized, (key, value, parent) => {
 ...
 */
export async function storeSecretSecurely(key, value, options = {}) {
 ...
 */
export async function getSecretSecurely(key) {
    // Try keychain first
