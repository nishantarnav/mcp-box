 ...
 */
export function expandEnvVars(filePath) {
 ...
 */
export function expandWindowsEnvVars(filePath) {
    if (!IS_WINDOWS)
        return filePath;
    return filePath.replace(/%([^%]+)%/g, (match, varName) => {
        return process.env[varName] || match;
 ...
 */
export function resolvePlatformPath(filePath) {
 ...
 */
export function getAgentConfigPath(agent, scope = 'user') {
 ...
 */
export function getAgentConfigPaths(agent) {
    switch (agent) {
        case 'claude':
            return {
 ...
 */
export function getMCPBoxConfigDir() {
    if (IS_WINDOWS) {
 ...
 */
export function getMCPBoxDataDir() {
    if (IS_WINDOWS) {
 ...
 */
export function getMCPBoxCacheDir() {
    if (IS_WINDOWS) {
 ...
 */
export async function isDirWritable(dirPath) {
    try {
 ...
 */
export async function findConfigFile(agent, scope = 'user') {
    const paths = getAgentConfigPaths(agent);
    const checkPaths = scope === 'project'
        ? [paths.project, paths.user].filter(Boolean)
 ...
 */
export function getTempFilePath(prefix = 'mcp-box', extension = '.tmp') {
 ...
 */
export async function cleanTempFiles(maxAge = 24 * 60 * 60 * 1000) {
 ...
 */
export function createSafeFilename(input) {
    return input
        .replace(/[<>:"/\\|?*]/g, '_')
        .replace(/\s+/g, '_')
        .replace(/_+/g, '_')
