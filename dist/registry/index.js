...
export class MCPRegistryManager {
     ...
     */
    async searchServers(filters, options = {}) {
        await this.ensureLoaded();
        if (!this.registry) {
            throw new Error('Registry not loaded');
        }
        const { query = '', tags = [], classification = [], transportTypes = [], supportedPlatforms = [], maintainer, category, license, minDownloads, dockerSupport, hasNpmPackage } = filters;
        const { fuzzy = true, limit = 50, offset = 0, sortBy = 'relevance', sortOrder = 'desc' } = options;
        let results = [];
        // Filter servers
     ...
     */
    calculateRelevanceScore(server, query, fuzzy) {
        const queryLower = query.toLowerCase();
        let score = 0;
        // Exact matches
        if (server.name.toLowerCase().includes(queryLower))
            score += 10;
        if (server.title.toLowerCase().includes(queryLower))
            score += 8;
        if (server.description.toLowerCase().includes(queryLower))
            score += 5;
        if (server.maintainer.toLowerCase().includes(queryLower))
            score += 3;
        if (server.tags.some(tag => tag.toLowerCase().includes(queryLower)))
            score += 4;
        ...
        if (server.classification === 'official')
            score += 2;
        return score;
    }
    /**
     ...
     */
    parseDownloads(downloadsStr) {
        if (!downloadsStr || downloadsStr === 'N/A')
            return 0;
     ...
     */
    async checkServerHealth(serverId, agent) {
     ...
     */
    async checkAllServersHealth(agent) {
        const servers = await this.getAllServers();
        return Promise.all(servers.map(server => this.checkServerHealth(server.id, agent)));
    }
}
