...
import { z } from 'zod';
import { getAgentConfigPath, findConfigFile, ensureDir, pathExists, isReadable, isWritable } from '../utils/paths.js';
import { configManager } from '../utils/config.js';
export class BaseAgentAdapter {
    /**
     * Get configuration file path
     */
    getConfigPath(scope = 'user') {
        return getAgentConfigPath(this.name, scope);
    }
    /**
     * Read agent configuration
     */
    async read(scope = 'user') {
     ...
     */
    async restore(backupPath, scope = 'user') {
     ...
     */
    async addServer(serverId, serverConfig, scope = 'user') {
        try {
     ...
     */
    async removeServer(serverId, scope = 'user') {
        try {
            // Read current configuration
            const config = await this.read(scope);
            const servers = this.extractServers(config);
     ...
     */
    async updateServer(serverId, serverConfig, scope = 'user') {
     ...
     */
    async getInstalledServers(scope = 'user') {
        try {
            const config = await this.read(scope);
            const servers = this.extractServers(config);
     ...
     */
    async checkPermissions(scope = 'user') {
        const configPath = this.getConfigPath(scope);
     ...
     */
    async getConfigStats(scope = 'user') {
        const configPath = this.getConfigPath(scope);
        const exists = await pathExists(configPath);
        let size;
        let modified;
     ...
     */
    async mergeConfig(sourceConfig, scope = 'user', overwrite = false) {
        try {
     ...
     */
    async exportConfig(scope = 'user') {
        return await this.read(scope);
    }
    /**
     * Import configuration from external source
     */
    async importConfig(config, scope = 'user', merge = true) {
        if (merge) {
